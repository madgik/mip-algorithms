...
def createColumnList(variablesMetadataPath):
	...
	metadataDictionary = []
	metadataDictionary = addGroupVariablesToList(metadataJSON,metadataDictionary)
...
def addGroupVariablesToList(groupMetadata,metadataDictionary):
	...
	if 'groups' in groupMetadata:
		for group in groupMetadata['groups']:
			metadataDictionary = addGroupVariablesToList(group,metadataDictionary)
	return metadataDictionary
...
def main():
	...
	for file in os.listdir(os.path.abspath(args.folderPath)):
		if file.endswith('.csv'):
			filePath = os.path.join(os.path.abspath(args.folderPath), file)
			reader = csv.reader(open(filePath, newline=''), dialect='excel')
			print('Splitting file: ' + filePath)
			headerDict = headerToDict(next(reader))
			for row in reader:
				harmonizedRow = harmonizeRow(headerDict,row,harmonisedColumnList)
